...
<style>
    ...
    .metric-lbl { font-size: 0.72rem; color: #6bab80; margin-top: 3px; }
    ...
    .info-box {
        background: #1a2a1a; border: 1px solid #4a7c59;
        border-radius: 8px; padding: 10px 16px;
        font-family: monospace; font-size: 13px;
        color: #a8d8a8; margin-bottom: 12px;
    }
...
CP_COEFF = {
    'H2':   [392.8422,  2.4906,  -3.6262,  -1.9624,  35.6197,  -81.3691,   62.6668],
    'CH4':  [1530.8043, 4.2038, -16.615,   -3.5668,  43.0563,  -86.5507,   65.5986],
    'CO':   [407.9796,  3.5028,   2.8524,  -2.3018,  32.9055, -100.1815,  106.1141],
    'CO2':  [514.5073,  3.4923,  -0.9306,  -6.0861,  54.1586,  -97.5157,   70.9687],
    'C2H4': [765.3491,  4.9341,  -7.9136, -11.0137,  74.3661, -127.7199,   86.5313],
    'C2H6': [903.4113,  4.4815,  11.6905,   8.4792, -77.0215,  122.9766,  -74.06  ],
    'H2O':  [706.3032,  5.1703,  -6.0865,  -6.6011,  36.2723,  -63.0965,   46.2085],
}
...
CLR = dict(
    ...
    bg='#0f1117',      text='#e8e8e8',   grid='#2a2a3e',
)
...
#  VDI cp FUNCTION
# ──────────────────────────────────────────────────────────────────────────────
def cp_vdi(compound, T_mean_K):
    a = CP_COEFF[compound]
    M = MOLAR_MASS[compound]
    tau = T_mean_K / (a[0] + T_mean_K)
    cp_mol = 8.314 * (a[1] + (a[2] - a[1]) * tau**2 *
             (1 - (a[0] / (a[0] + T_mean_K)) *
              (a[3] + a[4]*tau + a[5]*tau**2 + a[6]*tau**3)))
    return cp_mol / M
...
def run_model(cel_pct, hemi_pct, lig_pct, ash_pct, HTT):
    ...
    denom = sum(v * m for v, m in zip(raw_vol, MM))
    vol_pct = [v * m / denom * 100 for v, m in zip(raw_vol, MM)]
    ...
    T_mean_K = (HTT + 25) / 2 + 273.15
    cp_H2O   = cp_vdi('H2O', T_mean_K)
    ...
    O14  = sum(wt_pct[i] * cp_vdi(GAS_SPECIES[i], T_mean_K) for i in range(6)) / 100
